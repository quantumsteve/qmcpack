...
private:
  ...
  void generateSamples(const Real weight, ParticleSet& pset_target, RandomBase<Real>& rng, int steps = 0);
  void generateUniformGrid(RandomBase<Real>& rng);
  void generateUniformSamples(RandomBase<Real>& rng);
   ...
   *   *
   */
  void generateDensitySamples(bool save, int steps, RandomBase<Real>& rng, ParticleSet& pset_target);
  ...
  Position diffuse(const Real sqt, RandomBase<Real>& rng);
   ...
   */
  void warmupSampling(ParticleSet& pset_target, RandomBase<Real>& rng);
