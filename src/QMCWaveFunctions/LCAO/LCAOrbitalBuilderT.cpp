...
template<class T>
LCAOrbitalBuilderT<T>::LCAOrbitalBuilderT(ParticleSet& els, ParticleSet& ions, Communicate* comm, xmlNodePtr cur)
    ...
    if (file_exists)
    {
      ...
      for (int orb_idx = 0; orb_idx < orbital_set_size; orb_idx++)
        for (int center_idx = 0; center_idx < num_centers; center_idx++)
          broadcastCuspInfo(info(center_idx, orb_idx), *this->myComm, 0);
#endif
    }
    else
    {
      generateCuspInfo(info, tmp_targetPtcl, sourcePtcl, lcwc.lcao, spo_name, *this->myComm);
      if (this->myComm->rank() == 0)
