...
void VMCBatched::advanceWalkers(const StateForThread& sft,
                                ...
                                ContextForSteps<QMCTraits::RealType>& step_context,
                                bool recompute,
...
void VMCBatched::runVMCStep(int crowd_id,
                            ...
                            DriverTimers& timers,
                            std::vector<std::unique_ptr<ContextForSteps<QMCTraits::RealType>>>& context_for_steps,
...
bool VMCBatched::run()
{
  ...
  if (qmcdriver_input_.get_warmup_steps() > 0)
  {
    ...
    auto runWarmupStep = [](int crowd_id, StateForThread& sft, DriverTimers& timers,
                            UPtrVector<ContextForSteps<QMCTraits::RealType>>& context_for_steps,
