...
template<class T>
LCAOrbitalBuilderT<T>::LCAOrbitalBuilder(ParticleSet& els, ParticleSet& ions, Communicate* comm, xmlNodePtr cur)
...
template<class T>
std::unique_ptr<BasisSet_t> LCAOrbitalBuilderT<T>::loadBasisSetFromXML(xmlNodePtr cur, xmlNodePtr parent)
{
...
template<class T>
std::unique_ptr<BasisSet_t> LCAOrbitalBuilderT<T>::loadBasisSetFromH5(xmlNodePtr parent)
{
...
std::unique_ptr<SPOSetT<T>> LCAOrbitalBuilderT<T>::createSPOSetFromXML(xmlNodePtr cur)
{
  ...
  if (doCuspCorrection)
  {
    ...
    auto lcwc =
        std::make_unique<LCAOrbitalSetWithCorrectionT<T>>(spo_name, sourcePtcl, targetPtcl, std::move(myBasisSet));
...
void LCAOrbitalBuilderT<T>::readRealMatrixFromH5(hdf_archive& hin,
                                                 ...
                                                 Matrix<LCAOrbitalBuilderT<T>::RealType>& Creal) const
{
