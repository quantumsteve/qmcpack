...
public:
  ...
  WalkerSet& getWalkerSet(TaskGroup_& TG, const std::string& ID, RandomBase<QMCTraits::RealType>& rng)
  {
    auto xml = xmlBlocks.find(ID);
    if (xml == xmlBlocks.end())
...
protected:
  ...
  WalkerSet buildHandler(TaskGroup_& TG, xmlNodePtr cur, RandomBase<QMCTraits::RealType>& rng)
  {
