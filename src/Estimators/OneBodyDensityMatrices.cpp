...
inline void OneBodyDensityMatrices::generateUniformGrid(RandomBase<Real>& rng)
{
  Position rp;
  Position ushift = 0.0;
...
inline void OneBodyDensityMatrices::generateUniformSamples(RandomBase<Real>& rng)
{
  Position rp;
  for (int s = 0; s < samples_; ++s)
  {
...
OneBodyDensityMatrices::Position OneBodyDensityMatrices::diffuse(const Real sqt, RandomBase<Real>& rng)
{
  Position diff;
...
void OneBodyDensityMatrices::warmupSampling(ParticleSet& pset_target, RandomBase<Real>& rng)
{
  if (sampling_ == Sampling::METROPOLIS)
  {
    if (!warmed_up_)
