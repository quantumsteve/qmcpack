...
struct einspline_spo
{
  ...
  /** copy constructor
       ...
       */
  einspline_spo(einspline_spo& in, int ncrews, int crewID) : Owner(false), Lattice(in.Lattice)
  {
...
#pragma omp parallel for
  ...
  void set(int nx, int ny, int nz, int num_splines, int nblocks, bool init_random = true)
  {
    ...
    if (einsplines.empty())
    {
      ...
      pos_type end(1);
      einsplines.resize(nBlocks);
      auto myrandom = createRandomGenerator(11);
      Array<double, 3> data(nx, ny, nz);
      std::generate(data.begin(), data.end(), std::ref(*myrandom));
